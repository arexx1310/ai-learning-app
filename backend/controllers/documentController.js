...
export const uploadDocument = async (req, res, next) => {
    try{
        ...
        if( !title ){
            ...
            return res.status(400).json({
                success: false,
                error: 'Please provide a document titile',
                statusCode: 400
            });
        }
            ...
            const document = await Document.create({
                ...
                filePath: fileUrl, //Stor the URL instead of the local path
...
const processPDF = async ( documentId, filePath) => {
    try {
        ...
        console.log(`Document ${documentId} processed successfuly`);
    } catch (error) {
    ...
    try {
        ...
        if (relativePath) {
            ...
            await fs.unlink(absolutePath).catch((err) => {
                console.warn("File already gone or path incorrect:", absolutePath);
            });
        }
