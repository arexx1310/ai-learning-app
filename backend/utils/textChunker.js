/** 
 * Split the text into chunks for better AI processing
...
export const chunkText = (text, chunkSize = 500 ,overlap = 50) => {
    ...
    //Try to split by paragrpahs (single or double newlines)
...
/**
 * Fine relevant chunks based on keyword matching
 * @param {Array<Object>} chunks - Array of chunks
 * @param {string} query - Search Query
 * @param {number} maxChunks - Maximum cuhhnks to return
...
export const findRelevantChunks = (chunks, query, maxChunks = 3) => {
    ...
    const scoredChukns = chunks.map( (chunk, index) => {
        ...
        for(const word of queryWords) {
            ...
            const extactMatches =(content.match( new RegExp(`\\b${word}\\b`, 'g')) || []).length;
            score += extactMatches *3;
            
            //Partial match (lower case)

            const partialMatches = (content.match(new RegExp(word, 'g')) || []).length;
            score += Math.max(0, partialMatches - extactMatches) * 1.5;
        }
